...
function renderVerification(id) {
  ...
  if (isTeacherMode(s)) {
    box.innerHTML = `
      <section class="panel game-panel">
        ...
        <p>Modo de revisión activo. Esta fase queda accesible sin validación previa ni necesidad de realizar el test.</p>
      </section>
    `;
    return;
  }
  ...
  box.innerHTML = `
    <section class="panel game-panel">
      <h2>Validación de la práctica</h2>
      <p>Cuando hayas subido la práctica a EVAGD, introduce la clave facilitada por el profesorado para activar el test de esta fase.</p>
      <div class="verify-row">
        <input type="password" id="verify-input" placeholder="Introducir clave" ${done ? "disabled" : ""}>
  ...
  if (!done) {
    document.getElementById("verify-btn").onclick = () => {
      ...
      if (val === getTodayCode()) {
        ...
        renderTest(id);
        msg.textContent = "Clave correcta. Test desbloqueado.";
      } else {
        msg.textContent = "Clave incorrecta. Revisa la facilitada por el profesorado.";
      }
    };
  }
}
...
function renderCertificate() {
  ...
  const rows = APP.phases
    .map((p) => {
      if (isTeacherMode(s)) {
        return `<tr><td>${p.title}</td><td>Revisión</td><td>—</td><td>Acceso directo</td></tr>`;
      }
