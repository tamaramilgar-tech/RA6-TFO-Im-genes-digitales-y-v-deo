...
if (APP?.questions?.["4"]) {
  APP.questions["4"] = APP.questions["4"].map((q) =>
    q[0] === "¿Para qué sirve una transición en OpenShot?"
      ? ["¿Para qué sirve una transición en un editor de vídeo?", q[1], q[2]]
      : q
  );
}
...
function wireVerificationInput(inputId, handler) {
...
function showCorrectionForAttempt(attempt, idx) {
  let html = `<div class="correction"><h3>Revisión del intento ${idx + 1}</h3>`;
  attempt.questions.forEach((q, i) => {
    const ok = attempt.answers[i] === q.answer;
    html += `
      <div class="corr-item ${ok ? "ok" : "ko"}">
        <p><strong>${i + 1}. ${q.q}</strong></p>
        <p>Tu respuesta: ${q.options[attempt.answers[i]] || "Sin responder"}</p>
...
function approveAdvanceAfterFail(id) {
  const input = document.getElementById(`advance-code-${id}`);
  const msg = document.getElementById(`advance-msg-${id}`);
  ...
  if (value === VERIFY_CODE) {
    ...
    s.completed[id] = true;
    if (!s.results[id]) {
      s.results[id] = { status: "No superado", passed: false, score: s.best[id] || 0, percent: s.percent[id] || 0 };
    }
    saveState(s);
    renderHUD();
    renderTest(id);
...
function renderVerification(id) {
  ...
  if (isTeacherMode(s)) {
    box.innerHTML = `
      <section class="panel game-panel">
        ...
        <p>Esta fase queda accesible sin validación ni realización del test.</p>
      </section>
    `;
    return;
  }
  ...
  box.innerHTML = `
    <section class="panel game-panel">
      ...
      <p>Cuando hayas subido la práctica a EVAGD, introduce el código de validación para activar el test de esta fase.</p>
  ...
  if (!done) {
    ...
    wireVerificationInput("verify-input", submitVerification);
  }
}
...
function renderTest(id) {
  ...
  if (s.completed[id]) {
    ...
    box.innerHTML = `
      <section class="panel game-panel">
        ...
        <p>${s.passed[id] ? "Ya puedes continuar la aventura." : "La fase queda registrada como no superada."}</p>
        ${s.passed[id] || s.advanceApproved[id] ? nextPhaseButton(id) : ""}
      </section>
    `;
    return;
  }
  ...
  if (used >= MAX_ATTEMPTS && !s.passed[id]) {
    box.innerHTML = `
      <section class="panel game-panel">
        <h2>Test cerrado</h2>
        <p>Ya has agotado los ${MAX_ATTEMPTS} intentos. Revisa la corrección en el panel inferior.</p>
      </section>
    `;
    return;
  }
...
function renderSummary(id) {
  ...
  if (s.passed[id]) {
    ...
    html += `<div class="banner warn">No has superado la fase tras ${MAX_ATTEMPTS} intentos. Revisa las respuestas correctas. Para avanzar a la siguiente fase, será necesaria la validación docente.</div>`;
    attempts.forEach((a, idx) => {
      html += showCorrectionForAttempt(a, idx);
    });
    if (s.advanceApproved[id]) {
      html += `<div class="banner success">Avance autorizado. La fase queda registrada como no superada y ya puedes continuar.</div>${nextPhaseButton(id)}`;
    } else {
      html += `
        <div class="verify-row advance-row">
          <input type="password" id="advance-code-${id}" placeholder="********" autocomplete="off">
          <button class="btn pixel-btn" id="advance-btn-${id}" type="button">Autorizar avance</button>
        </div>
        <p class="mini" id="advance-msg-${id}"></p>
      `;
    }
  } else if (remaining > 0) {
  ...
  if (!s.passed[id] && attempts.length >= MAX_ATTEMPTS && !s.advanceApproved[id]) {
    const btn = document.getElementById(`advance-btn-${id}`);
    if (btn) btn.onclick = () => approveAdvanceAfterFail(id);
    wireVerificationInput(`advance-code-${id}`, () => approveAdvanceAfterFail(id));
  }
}
...
function renderCertificate() {
  ...
  const rows = APP.phases
    .map((p) => {
      if (isTeacherMode(s)) {
        return `<tr><td>${p.title}</td><td>—</td><td>Revisión docente</td><td>—</td><td>Acceso directo</td></tr>`;
      }
      ...
      return `<tr><td>${p.title}</td><td>${s.verified[p.id] ? "Sí" : "No"}</td><td>${result.status}</td><td>${result.percent}%</td><td>${s.passed[p.id] ? "Sí" : "No"}</td></tr>`;
    })
    .join("");
